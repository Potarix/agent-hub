...
function runSSHCommand(agent, command, timeout = 120000, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    let wrappedCommand;
    if (options.singleQuoteWrap) {
      // Single-quote wrapping prevents the outer shell from expanding $(), backticks, etc.
      const escaped = command.replace(/'/g, "'\\''");
      wrappedCommand = `bash -l -c '${escaped}'`;
    } else {
      wrappedCommand = `bash -l -c ${JSON.stringify(command)}`;
    }
