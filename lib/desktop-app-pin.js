...
// True iff the foreign app's process is the system-wide frontmost app right
// now. Used to decide whether a user click landed in the pinned app's window.
// Uses the same `process "${processName}"` reference the pin code uses, so
// it works wherever pinning works — avoids fragile string-matching against
// the OS's reported frontmost-process name (which can differ in case,
// helpers, or localized variants).
async function isFrontmost(key) {
  const { processName } = getApp(key);
  const out = await runOsascriptPromise(`
    tell application "System Events"
      try
        return frontmost of process "${processName}"
      on error
        return false
      end try
    end tell
